...
def test_package(global_config, package_name, do_unit_tests=True):
    pdir = Path(package_name)
    conanfile = pdir / "conanfile-static.py"
    if not conanfile.is_file():
        conanfile = pdir / "conanfile.py"
    test_dir = global_config[prog_path.stem]["scratch-folder"]
    source_dir = test_dir
    build_dir = Path(source_dir) / pdir / "build"
    outfile = Path(test_dir) / f"{package_name}.out"
...
if __name__ == "__main__":
    ...
    with (Path(pc.config[prog_path.stem]["scratch-folder"]) / "conan_export.out").open(
        "w"
    ) as f:
    ...
    results = [test_package(pc.config, package, args.do_unit_tests) for package in packages_to_test]
    print("Done")
